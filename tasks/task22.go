...
// задаем костанты для типов операций
// и некоторых символом (чтобы не было magic numbers)
const (
    add string = "+"
    sub string = "-"
    mul string = "x"
...
// создаем интерфейс для операции
// с числами
...
// создаем типы операций, для int & float
// раелизация операций с числами больше 2^20
// с помощью пакета big
...
// создаем валидатор для чисел,
// проверяем что все символы в стороке - числа
...
// проверяем, является ли число типом float
...
// проверяем, является ли число типом int
...
// читаем данные из терминала (командной строки)
func ReadCLI() args {
    var a args
    if len(os.Args) < 4 {
        log.Fatal("Not enough args...")
    }
    // передаем все аргуметы, кроме названия программы
    return append(a, os.Args[1:]...)
}

// выполняем операцию, в зависимости от её типа
